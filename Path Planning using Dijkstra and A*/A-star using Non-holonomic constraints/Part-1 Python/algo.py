...
# Node class containing action set, graph generation and A-star Algorithm
class Node():
    ...
    def move1(self, node, cost):
        ...
        cost2go = self.weight * ((self.goal_node[0] - new_node[0]) ** 2 + (self.goal_node[1] - new_node[1]) ** 2) ** (
                    1 / 2)
    ...
    def child_generator(self, node, cost):
        ...
        # List to store all children nodes
        ...
        for cost in childs_cost.keys():
            if utils.check_node(childs_cost[cost], self.clearance) == True and \
                    visited_nodes[int(round(childs_cost[cost][0], 1) / 0.2)][int(round(childs_cost[cost][1], 1) / 0.2)][
                        int(round(childs_cost[cost][2], 1) / 10)] == 0:
                valid_children.append(
                    (cost, childs_cost[cost], childs_cost[cost][4], self.index(childs_cost[cost]), node))
                valid_childs_dict[self.index(childs_cost[cost])] = [childs_cost[cost], node, self.index(node)]
                visited_nodes[int(round(childs_cost[cost][0], 1) / 0.2)][int(round(childs_cost[cost][1], 1) / 0.2)][
                    int(round(childs_cost[cost][2], 1) / 10)] = 1
    ...
    def astar(self):
        ...
        while len(explored_nodes) > 0:
            ...
            itr = itr + 1
            if ((min_cost_child[0] - self.goal_node[0]) ** 2 + (
                    min_cost_child[1] - self.goal_node[1]) ** 2) <= 0.25 ** 2:
